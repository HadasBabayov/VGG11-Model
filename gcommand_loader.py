...
def make_dataset(dir, class_to_idx, search_subdir):
    spects = []
    ...
    if search_subdir:
        ...
        for root, _, fnames in sorted(os.walk(dir)):
            for fname in sorted(fnames, key=lambda x: int(x.partition('.')[0])):
                if is_audio_file(fname):
                    spects.append((os.path.join(root, fname), 0))
    return spects
...
class GCommandLoader(data.Dataset):
    ...
    def __init__(self, root, search_subdir=True, transform=None, target_transform=None, window_size=.02,
                 window_stride=.01, window_type='hamming', normalize=True, max_len=101):
        classes, class_to_idx = find_classes(root)
        spects = make_dataset(root, class_to_idx, search_subdir)
        if len(spects) == 0:
        ...
        self.search_subdir = search_subdir
